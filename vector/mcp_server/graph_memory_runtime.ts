...
function upsertGraphEdge(graph: GraphMemoryLike, edge: Omit<GraphEdgeLike, "first_seen_at" | "last_seen_at">, now?: string): void {
  const recordedAt = timestamp(now);
...
function providerRunIdsForEvidenceIds(researchMemo: any, evidenceIds: string[]): string[] {
  const lookup = new Set(evidenceIds);
  const ids = (researchMemo?.evidence_table ?? [])
    .filter((item: any) => lookup.has(item.id) && typeof item.provider_run_id === "string" && item.provider_run_id.trim())
    .map((item: any) => item.provider_run_id as string);
  return [...new Set<string>(ids)];
}

function evidenceIdsForKinds(researchMemo: any, kinds: string[]): string[] {
  return (researchMemo?.evidence_table ?? [])
    .filter((item: any) => kinds.includes(item.kind))
    .map((item: any) => item.id);
}

function snapshotProvenance(state: any, now?: string, evidenceIds: string[] = [], providerRunIds: string[] = []) {
  return {
...
export function syncGraphFromState(state: any, graphMemory: GraphMemoryLike, action: string, now?: string): GraphMemoryLike {
  const recordedAt = timestamp(now);
  const graph: GraphMemoryLike = {
    ...graphMemory,
    nodes: [...graphMemory.nodes],
    edges: [...graphMemory.edges],
    sync_history: [...graphMemory.sync_history],
  };
  const channelEvidenceIds = state.research_memo?.channel_observations.flatMap((item: any) => item.evidence_ids) ?? [];
  const venueEvidenceIds = state.research_memo?.venue_observations.flatMap((item: any) => item.evidence_ids) ?? [];
  ...
  const channelNames = [...new Set([
    ...(state.channel_scores ?? []).map((item: any) => item.channel),
    ...(state.channel_selected ? [state.channel_selected] : []),
  ])];
  for (const channel of channelNames) {
    const observation = state.research_memo?.channel_observations.find((item: any) => item.channel.toLowerCase() === channel.toLowerCase());
    ...
    upsertGraphNode(graph, {
      ...
      attributes: {
        ...
        score: (state.channel_scores ?? []).find((item: any) => item.channel === channel)?.score ?? null,
      },
      provenance: [snapshotProvenance(state, recordedAt, evidenceIds, providerRunIdsForEvidenceIds(state.research_memo, evidenceIds))],
    }, recordedAt);
    upsertGraphEdge(graph, {
  ...
  for (const venue of venueNames) {
    const observation = state.research_memo?.venue_observations.find((item: any) => item.venue.toLowerCase() === venue.toLowerCase());
    const evidenceIds = observation?.evidence_ids ?? [];
  ...
  const trustSignals = mergeUniqueStrings(
    state.research_memo?.trust_signals,
    state.venue_card?.trust_signal_needed ? [state.venue_card.trust_signal_needed] : [],
  );
...
export function queryGraph(graph: GraphMemoryLike, query: { node_id?: string; entity_type?: string; search?: string; limit?: number }) {

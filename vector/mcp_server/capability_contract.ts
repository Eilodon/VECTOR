...
export type CapabilityToolset = keyof typeof CAPABILITY_TOOLSETS;
...
const SAFE_MODE_BLOCKED_TOOLS = new Set<string>([
  "vector_update_state",
  "vector_undo",
]);

const TOOLSET_NAMES = Object.keys(CAPABILITY_TOOLSETS) as CapabilityToolset[];
const TOOL_POLICIES = new Map<string, CapabilityPolicy>();
...
export function allCapabilityToolsets(): CapabilityToolset[] {
  return [...TOOLSET_NAMES];
}

export function capabilityPolicy(toolName: string): CapabilityPolicy | null {
  return TOOL_POLICIES.get(toolName) ?? null;
}

export function listToolsetTools(toolset: CapabilityToolset): string[] {
  return [...CAPABILITY_TOOLSETS[toolset]];
}

export function resolveToolsets(requested?: string[] | null): CapabilityToolset[] {
  if (!requested?.length) {

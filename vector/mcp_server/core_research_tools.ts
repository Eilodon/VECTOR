export function registerResearchTools(deps: {
  ...
  now: () => string;
  listSearchProviders: () => any[];
  getSearchProvider: (providerId: string) => any;
  defaultSearchProviderId: () => string;
  mergeResearchMemoFromProvider: (existing: any, query: string, providerRun: any, payloads: any[], updatedAt: string) => any;
  mergeBoundedEvidence: (existing: any[], incoming: any[], maxEntries?: number) => any[];
  maxResearchEvidenceItems: number;
  zod: any;
  evidenceItemSchema: { parse: (value: unknown) => any };
  evidenceKindSchema: any;
  sourceTypeSchema: any;
  evidenceStrengthSchema: any;
  researchProviderRunSchema: any;
  researchMemoSchema: any;
  researchChannelObservationSchema: any;
  researchVenueObservationSchema: any;
  staleAfterDaysFor: (sourceType: any) => number;
  scoreChannel: (channel: string, stageHint?: string, researchMemo?: any) => any;
  mergeUniqueStrings: (...collections: Array<string[] | undefined>) => string[];
  deriveWhiteSpace: (competitors: string[], substitutes: string[], workflowCompetitors: string[], attentionCompetitors: string[]) => string[];
  upsertChannelObservation: (observations: any[], observation: any) => any[];
  defaultResearchMemo: (question: string) => any;
  researchDimensionSchema: any;
}) {
  ...
  deps.registerVectorTool(
    ...
    async ({ query, provider, source_whitelist, max_results }: any) => {
      ...
      const cleaned = deps.sanitizeRecursive({ query, provider, source_whitelist, max_results }) as {
        query: string;
        provider?: string;
      ...
      const nextResearchMemo = deps.researchMemoSchema.parse(
        deps.mergeResearchMemoFromProvider(existing, cleaned.query, providerRun, searchResponse.results, deps.now()),
      );
      const affectedChannels = [...new Set(nextResearchMemo.channel_observations
        .filter((item: any) => item.provider_run_id === providerRun.id)
        .map((item: any) => item.channel))] as string[];
      ...
      return deps.emit(
        "vector_research_search",
        {
          ...
          decisions: [
            ...
            `Channel observations derived: ${nextResearchMemo.channel_observations.filter((item: any) => item.provider_run_id === providerRun.id).length}`,
            `Venue observations derived: ${nextResearchMemo.venue_observations.filter((item: any) => item.provider_run_id === providerRun.id).length}`,
          ],
          next_actions: previewScores.length
            ? previewScores.map((score: any) => `Preview ${score.channel}: ${score.score}/100`)
  ...
  deps.registerVectorTool(
    ...
    async ({ question, sources }: any) => {
      ...
      const cleaned = deps.sanitizeRecursive({ question, sources }) as { question: string; sources: any[] };
      ...
      return deps.emit(
        "vector_source_capture",
        {
          ...
          decisions: [
            ...
            `Latest source types: ${deps.mergeUniqueStrings(normalizedSources.map((item: any) => item.source_type)).join(", ")}`,
          ],
          next_actions: [
  ...
  deps.registerVectorTool(
    ...
    async (args: any) => {
      ...
      const marketMemo = {
        ...
        white_space: deps.deriveWhiteSpace(
  ...
  deps.registerVectorTool(
    ...
    async (args: any) => {
      ...
      const cleaned = deps.sanitizeRecursive(args) as any;
      const normalizedEvidenceTable = (cleaned.evidence_table ?? []).map((item: any) => deps.evidenceItemSchema.parse({
        ...item,
        stale_after_days: item.stale_after_days ?? deps.staleAfterDaysFor(item.source_type),
      }));

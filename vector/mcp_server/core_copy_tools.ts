export function registerCopyTools(deps: {
  ...
  getState: () => any;
  buildSalesCopyPack: (state: any, cleaned: any) => { sales_copy: any; objection_map: any };
  reviewSalesCopyPack: (state: any, reviewedAt: string) => any;
  now: () => string;
  requireShipReadyCopyReview: () => any;
  zod: any;
  mediaSpecShape: Record<string, any>;
}) {
      ...
      return {
        content: [{
          ...
          text: deps.artifactToText({
            title: "Media Spec",
            summary: `A structured spec for ${(cleaned as any).artifact_type} has been prepared for downstream rendering.`,
            decisions: [
              `Artifact type: ${(cleaned as any).artifact_type}`,
              `Copy review score: ${review.overall_score}/100`,
              `Approved first variant: ${review.first_test_variant}`,
              `Style keys: ${Object.keys((cleaned as any).style_spec ?? {}).join(", ") || "none"}`,
            ],

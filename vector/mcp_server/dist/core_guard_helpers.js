...
export function sanitizeRecursive(value, depth = 0) {
    const banned = [
...
export function ensurePhaseAllowed(currentPhase, expected) {
    if (!expected.includes(currentPhase)) {
        throw new Error(`Phase guard failed. Current phase is '${currentPhase}', but this tool expects one of: ${expected.join(", ")}.`);
    }
}
export function ensureToolPhase(toolName, currentPhase, toolPolicy) {
...
export function requireShipReadyCopyReview(state) {
    if (!state.sales_copy) {

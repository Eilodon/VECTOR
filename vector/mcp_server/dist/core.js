...
import { mergeResearchMemoFromProvider, researchChannelAdjustment, researchVenueAdjustment, staleAfterDaysFor, upsertChannelObservation, } from "./research_runtime.js";
...
import { WORKFLOW_MILESTONES, WORKFLOW_MODES, WORKFLOW_PHASES, WORKFLOW_STAGES, artifactKeyForAction, phasePolicy, toolPolicy, } from "./workflow_contract.js";
/**
...
export function createVectorRuntime(initialOptions) {
    ...
    function defaultRouting(phase = "intake") {
        return {
            persona: "unknown",
    ...
    function computeFounderEdgeScore(entry) {
        return [
    ...
    function upsertFounderEdgeAudit(audits, entry) {
    ...
    function mergeArtifactRegistry(base, action, artifact, phase) {
        const entry = {
    ...
    function syncCanonicalViews(state) {
    ...
    async function loadGraphMemory() {
        ...
        try {
            return VectorGraphMemorySchema.parse({
                ...
                ...loaded,
                version: CURRENT_VERSION,
                updated_at: loaded.updated_at ?? now(),
            });
        }
        catch (error) {
    ...
    async function saveGraphMemory(nextGraph) {
    ...
    function appendHistory(action, note = "") {
    ...
    async function commitState(nextState, action, note = "") {
        const run = async () => {
            ...
            if (action !== "vector_graph_sync" && action !== "vector_graph_query") {
                try {
                    const nextGraph = VectorGraphMemorySchema.parse(syncGraphFromState(VECTOR_STATE, GRAPH_MEMORY, `auto:${action}`, now()));
    ...
    function ensurePhaseAllowed(expected) {
        if (!expected.includes(VECTOR_STATE.phase)) {
            throw new Error(`Phase guard failed. Current phase is '${VECTOR_STATE.phase}', but this tool expects one of: ${expected.join(", ")}.`);
        }
    }
    function ensureToolPhase(toolName) {
        ...
        ensurePhaseAllowed([...policy.entry_phases]);
    }
    ...
    function validatePhaseTransition(nextPhase) {
        if (!nextPhase || nextPhase === VECTOR_STATE.phase)
            return;
        const allowed = phasePolicy(VECTOR_STATE.phase).allowed_next;
        if (!allowed.includes(nextPhase)) {
            throw new Error(`Illegal phase transition: ${VECTOR_STATE.phase} -> ${nextPhase}. Allowed next phases: ${allowed.join(", ") || "none"}.`);
        }
    }
    function roleHints() {
        return [
    ...
    function withRequestSchema(shape) {
        return {
    ...
    function registerRuntimeTools() {
        ...
        for (const definition of TOOL_DEFINITIONS) {
            ...
            }, withIdempotency(definition.name, async (args) => {
    ...
    function signalItem(label, source = "manual", confidence = 0.7, notes = "") {
        return {
    ...
    function scoreChannel(channel, stageHint, researchMemo) {
        const normalized = channel.toLowerCase();
        const heuristics = {
    ...
    function deriveWhiteSpace(competitors, substitutes, workflowCompetitors, attentionCompetitors) {
        const gaps = [];
        if (!competitors.length)
            gaps.push("No named direct competitors supplied; map the category before finalizing the thesis.");
        if (!substitutes.length)
            gaps.push("Find what the ICP uses instead of buying a dedicated solution.");
        if (!workflowCompetitors.length)
            gaps.push("Identify internal workflows or spreadsheets that are competing with the product.");
        if (!attentionCompetitors.length)
            gaps.push("Name the channels, creators, or tools stealing attention from the same ICP.");
        if (competitors.length && substitutes.length)
            gaps.push("Look for a lower-friction entry point than the direct competitor offers.");
        return gaps;
    }
    function mergeUniqueStrings(...collections) {
        return [...new Set(collections.flatMap((items) => items ?? []).map((item) => item.trim()).filter(Boolean))];
    }
    function hasStateField(candidateState, field) {
    ...
    function gateHasRequiredFields(state, gate) {
        switch (gate) {
            case "intake_cleared":
                return Boolean(state.product_description?.trim() && state.icp_hypothesis?.trim());
            case "icp_cleared":
                return Boolean(state.target_user?.trim()
            ...
            case "market_cleared":
                return Boolean(state.market.category_stance?.trim()
    ...
    function reconcileSessionState(loaded) {
    ...
    function phasePrerequisites(targetPhase, candidateState) {
    ...
    function assertPhasePrerequisites(targetPhase, candidateState) {
    ...
    function scoreVenue(venue, primaryChannel, stageHint, researchMemo) {
    ...
    async function emit(action, artifact, statePatch) {
        const nextPhase = (statePatch.phase ?? VECTOR_STATE.phase);
        const updatedRegistry = mergeArtifactRegistry(statePatch.artifact_registry ?? VECTOR_STATE.artifact_registry, action, artifact, nextPhase);
    ...
    // --- TOOLS ---
    registerVectorTool("vector_list_toolsets", {
    ...
    registerVectorTool("vector_list_toolset_tools", {
        description: "List the tools that belong to a VECTOR capability toolset.",
        inputSchema: {
            toolset: z.enum(allCapabilityToolsets()).describe("Capability toolset to inspect."),
        },
    }, async ({ toolset }) => {
        const tools = listToolsetTools(toolset).map((name) => {
        ...
        const enabled = CAPABILITY_STATE.toolsets.includes(toolset);
        return {
            content: [{ type: "text", text: artifactToText({
    ...
    registerVectorTool("vector_undo", {
    ...
    registerVectorTool("vector_intake", {
        ...
        const cleaned = sanitizeRecursive({ product_description, icp_hypothesis, is_live, product_name, product_price, product_category, builder_background, persona, intent, platform, mode, stage });
        const nextPhase = "icp";
        const nextState = {
            ...VECTOR_STATE,
            phase: nextPhase,
    ...
    registerVectorTool("vector_icp_jtbd", {
        ...
        const cleaned = sanitizeRecursive({ target_user, job_statement, riskiest_assumption, evidence, watering_holes, wtp_signal, trigger_moment, desired_outcome, push, pull, anxiety, habit, evidence_tags, confidence, top_unknown, next_experiment });
    ...
    registerVectorTool("vector_market_terrain", {
        ...
        const cleaned = sanitizeRecursive({ stage_confirmed, market_category, category_stance, competitors, substitutes, workflow_competitors, attention_competitors, gold_zone_channels, red_ocean, white_space_notes, why_now_pressure, next_research_question, notes });
        const marketMemo = {
        ...
        const nextPhase = VECTOR_STATE.founder_edge_audit.length ? "channel" : "market";
        const nextState = {
            ...VECTOR_STATE,
            phase: nextPhase,
            milestone: PHASE_TO_MILESTONE[nextPhase],
        ...
        return emit("vector_market_terrain", {
    ...
    registerVectorTool("vector_list_research_providers", {
    ...
    registerVectorTool("vector_research_search", {
        ...
        const cleaned = sanitizeRecursive({ query, provider, source_whitelist, max_results });
        ...
        const nextResearchMemo = ResearchMemoSchema.parse(mergeResearchMemoFromProvider(existing, cleaned.query, providerRun, searchResponse.results, now()));
        ...
        return emit("vector_research_search", {
    ...
    registerVectorTool("vector_source_capture", {
        ...
        const cleaned = sanitizeRecursive({ question, sources });
        ...
        return emit("vector_source_capture", {
    ...
    registerVectorTool("vector_competitor_map", {
        ...
        const cleaned = sanitizeRecursive(args);
        ...
        const marketMemo = {
            ...
            white_space: deriveWhiteSpace(nextResearchMemo.competitors, nextResearchMemo.substitutes, nextResearchMemo.workflow_competitors, nextResearchMemo.attention_competitors),
        ...
        return emit("vector_competitor_map", {
    ...
    registerVectorTool("vector_channel_evidence", {
        ...
        return emit("vector_channel_evidence", {
    ...
    registerVectorTool("vector_research_memo", {
        ...
        const marketMemo = {
            ...
            white_space: deriveWhiteSpace(researchMemo.competitors, researchMemo.substitutes, researchMemo.workflow_competitors, researchMemo.attention_competitors),
            unresolved_unknowns: researchMemo.unknowns,
        };
        const nextPhase = VECTOR_STATE.phase === "market" ? "channel" : VECTOR_STATE.phase;
        const nextState = {
            ...VECTOR_STATE,
            phase: nextPhase,
            milestone: PHASE_TO_MILESTONE[nextPhase],
        ...
        return emit("vector_research_memo", {
            title: "Research Memo",
    ...
    registerVectorTool("vector_founder_edge_audit", {
        ...
        const cleaned = sanitizeRecursive({ channels, assessments });
        ...
        return emit("vector_founder_edge_audit", {
    ...
    registerVectorTool("vector_channel_score", {
        ...
        const cleaned = sanitizeRecursive({ channels, stage_hint, constraints });
        ...
        return emit("vector_channel_score", {
    ...
    registerVectorTool("vector_venue_score", {
        ...
        const cleaned = sanitizeRecursive({ venues, primary_channel, stage_hint, constraints });
        ...
        return emit("vector_venue_score", {
    ...
    registerVectorTool("vector_route_context", {
        ...
        const cleaned = sanitizeRecursive({ persona, mode, platform, intent, reason });
        const nextState = {
            ...VECTOR_STATE,
            routing: {
        ...
        return emit("vector_route_context", {
    ...
    registerVectorTool("vector_strategy_map", {
        ...
        return emit("vector_strategy_map", {
            title: title ?? "Strategy Map",
    ...
    registerVectorTool("vector_graph_sync", {
        ...
        const graph = VectorGraphMemorySchema.parse(syncGraphFromState(VECTOR_STATE, GRAPH_MEMORY, reason ? `vector_graph_sync:${reason}` : "vector_graph_sync", now()));
    ...
    registerVectorTool("vector_graph_query", {
        ...
        const cleaned = sanitizeRecursive({ node_id, entity_type, search, limit });
    ...
    registerVectorTool("vector_sync_kb", {
    ...
    registerVectorTool("vector_thesis", {
    ...
    registerVectorTool("vector_venue", {
    ...
    registerVectorTool("vector_signal_review", {
        ...
        const cleaned = sanitizeRecursive({ green, yellow, red, sample_size, notes });
        ...
        return emit("vector_signal_review", {
    ...
    registerVectorTool("vector_sales_copy", {
        ...
        const cleaned = sanitizeRecursive({ desired_conversion_step, angle, headline, subheadline, body, cta, objections, followup_ladder });
    ...
    registerVectorTool("vector_copy_review", {
        ...
        return emit("vector_copy_review", {
            title: "Copy Review",
    ...
    registerVectorTool("vector_update_state", {
        ...
        const cleaned = sanitizeRecursive(patch);
        ...
        return emit("vector_update_state", {
    ...
    registerVectorTool("vector_state_snapshot", {
    ...
    registerVectorTool("vector_render_media", {

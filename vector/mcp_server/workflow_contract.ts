...
export const WORKFLOW_MILESTONES = ["M0", "M1", "M2", "M3", "M4", "M5", "M6"] as const;
export const WORKFLOW_STAGES = ["0→1", "1→10", "10→100"] as const;
...
export type WorkflowMilestone = typeof WORKFLOW_MILESTONES[number];
...
type PhasePrerequisiteRule = {
  kind: "allOf" | "anyOf";
  fields: readonly string[];
  message: string;
};

// [v2.0.0] Gate checklist — each phase must pass before advance
type PhaseGateChecklist = {
  required_conditions: readonly string[];
  kill_switch_check: string;
};
...
type PhasePolicy = {
  ...
  allowed_next: readonly WorkflowPhase[];
  artifact_key: WorkflowArtifactKey | null;
  prerequisites: readonly PhasePrerequisiteRule[];
  gate_checklist: PhaseGateChecklist; // [v2.0.0]
};

type ToolPolicy = {
  entry_phases: readonly WorkflowPhase[];
  emits_artifact: WorkflowArtifactKey | null;
  target_phase: WorkflowPhase | null;
};
...
export const WORKFLOW_CONTRACT = {
  ...
  } satisfies Record<WorkflowPhase, PhasePolicy>,
  tools: {
  ...
  } satisfies Record<string, WorkflowArtifactKey>,
} as const;

export function phasePolicy(phase: WorkflowPhase): PhasePolicy {
  return WORKFLOW_CONTRACT.phases[phase];
}

export function toolPolicy(toolName: string): ToolPolicy | null {
  const tools = WORKFLOW_CONTRACT.tools as Record<string, ToolPolicy>;
  return tools[toolName] ?? null;
}

export function artifactKeyForAction(action: string): WorkflowArtifactKey | null {
  const registry = WORKFLOW_CONTRACT.artifact_registry_by_action as Record<string, WorkflowArtifactKey>;
  return registry[action] ?? null;
}

// [v2.0.0] Gate validation helper
export function gateChecklistForPhase(phase: WorkflowPhase): PhaseGateChecklist {

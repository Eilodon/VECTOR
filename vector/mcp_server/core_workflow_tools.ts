export function registerWorkflowTools(deps: {
  ...
  modeSchema: any;
  stageSchema: any;
  evidenceTagSchema: any;
  confidenceLevelSchema: any;
  founderEdgeAuditEntrySchema: { parse: (value: unknown) => any };
  experimentEntrySchema: any;
  getCurrentVersion: () => string;
  now: () => string;
  phaseToStage: (phase: any) => any;
  inferModeForPhase: (phase: any) => any;
  defaultRouting: (phase?: any) => any;
  defaultSession: () => any;
  defaultProduct: () => any;
  defaultMarket: () => any;
  defaultDistribution: () => any;
  defaultICP: () => any;
  defaultArtifactRegistry: () => any;
  phaseToMilestone: Record<string, any>;
  renderStrategyMap: (state: any) => string;
  scoreChannel: (channel: string, stageHint?: string, researchMemo?: any) => any;
  scoreVenue: (venue: string, primaryChannel?: string, stageHint?: string, researchMemo?: any) => any;
  signalItem: (label: string, source?: string, confidence?: number, notes?: string) => any;
  deriveWhiteSpace: (competitors: string[], substitutes: string[], workflowCompetitors: string[], attentionCompetitors: string[]) => string[];
  founderEdgeAuditFor: (channel: string) => any;
  computeFounderEdgeScore: (entry: any) => number;
  upsertFounderEdgeAudit: (entries: any[], entry: any) => any[];
  ...
  deps.registerVectorTool(
    ...
    async ({ product_description, icp_hypothesis, is_live, product_name, product_price, product_category, builder_background, persona, intent, platform, mode, stage }: any) => {
      const cleaned = deps.sanitizeRecursive({ product_description, icp_hypothesis, is_live, product_name, product_price, product_category, builder_background, persona, intent, platform, mode, stage }) as any;
      const nextPhase = "icp";
      const nextState = {
        ...state(),
        phase: nextPhase,
  ...
  deps.registerVectorTool(
    ...
    async ({ target_user, job_statement, riskiest_assumption, evidence, watering_holes, wtp_signal, trigger_moment, desired_outcome, push, pull, anxiety, habit, evidence_tags, confidence, top_unknown, next_experiment }: any) => {
      ...
      const cleaned = deps.sanitizeRecursive({ target_user, job_statement, riskiest_assumption, evidence, watering_holes, wtp_signal, trigger_moment, desired_outcome, push, pull, anxiety, habit, evidence_tags, confidence, top_unknown, next_experiment }) as any;
  ...
  deps.registerVectorTool(
    ...
    async ({ stage_confirmed, market_category, category_stance, competitors, substitutes, workflow_competitors, attention_competitors, gold_zone_channels, red_ocean, white_space_notes, why_now_pressure, next_research_question, notes }: any) => {
      ...
      const cleaned = deps.sanitizeRecursive({ stage_confirmed, market_category, category_stance, competitors, substitutes, workflow_competitors, attention_competitors, gold_zone_channels, red_ocean, white_space_notes, why_now_pressure, next_research_question, notes }) as any;
      const marketMemo = {
  ...
  deps.registerVectorTool(
    ...
    async ({ channels, assessments }: any) => {
      ...
      const cleaned = deps.sanitizeRecursive({ channels, assessments }) as any;
      const assessmentLookup = new Map((cleaned.assessments ?? []).map((item: any) => [item.channel.toLowerCase(), item]));
      let nextAudits = [...state().founder_edge_audit];
      const results = cleaned.channels.map((channel: string) => {
        const existing = deps.founderEdgeAuditFor(channel);
        const provided: any = assessmentLookup.get(channel.toLowerCase());
      ...
      return deps.emit("vector_founder_edge_audit", {
        ...
        decisions: results.map((item: any) => `${item.channel}: ${item.score}/5 builder advantage`),
        next_actions: [
  ...
  deps.registerVectorTool(
    ...
    async ({ channels, stage_hint, constraints }: any) => {
      ...
      const cleaned = deps.sanitizeRecursive({ channels, stage_hint, constraints }) as any;
      const scored = cleaned.channels.map((channel: string) => deps.scoreChannel(channel, cleaned.stage_hint, state().research_memo));
      ...
      return deps.emit("vector_channel_score", {
        ...
        decisions: ranked.map((item: any) => `${item.channel}: ${item.score}/100 — ${item.reason}`),
  ...
  deps.registerVectorTool(
    ...
    async ({ venues, primary_channel, stage_hint, constraints }: any) => {
      ...
      const cleaned = deps.sanitizeRecursive({ venues, primary_channel, stage_hint, constraints }) as any;
      const scored = cleaned.venues.map((venue: string) => deps.scoreVenue(venue, cleaned.primary_channel, cleaned.stage_hint, state().research_memo));
      const ranked = [...scored].sort((a, b) => b.score - a.score);
      const selected = ranked[0] ?? null;
      const nextState = {
        ...state(),
        ...
        distribution: {
          ...
          alternatives: ranked.map((item: any) => item.venue),
        },
      ...
      return deps.emit("vector_venue_score", {
        ...
        decisions: ranked.map((item: any) => `${item.venue}: ${item.score}/100 — ${item.reason}`),
        next_actions: selected
          ? [
  ...
  deps.registerVectorTool(
    ...
    async ({ persona, mode, platform, intent, reason }: any) => {
      const cleaned = deps.sanitizeRecursive({ persona, mode, platform, intent, reason }) as any;
      const nextState = {
        ...state(),
        routing: {
  ...
  deps.registerVectorTool(
    ...
    async ({ note }: any) => {
      const cleaned = deps.sanitizeRecursive({ note }) as { note?: string };
  ...
  deps.registerVectorTool(
    ...
    async ({ green, yellow, red, sample_size, notes }: any) => {
      ...
      const cleaned = deps.sanitizeRecursive({ green, yellow, red, sample_size, notes }) as any;
      const positives = (cleaned.green ?? []).map((item: string) => deps.signalItem(item, "review", 0.8, "green"));
      const ambers = (cleaned.yellow ?? []).map((item: string) => deps.signalItem(item, "review", 0.6, "yellow"));
      const negatives = (cleaned.red ?? []).map((item: string) => deps.signalItem(item, "review", 0.3, "red"));

export function registerGraphTools(deps: {
  ...
  getState: () => any;
  getGraphMemory: () => any;
  saveGraphMemory: (graph: any) => Promise<void>;
  vectorGraphMemorySchema: { parse: (value: unknown) => any };
  syncGraphFromState: (state: any, graphMemory: any, action: string, now?: string) => any;
  queryGraph: (graph: any, query: any) => { nodes: any[]; edges: any[] };
  zod: any;
  graphEntityTypeSchema: any;
}) {
  deps.registerVectorTool(
    ...
    async ({ reason }: any) => {
      const graph = deps.vectorGraphMemorySchema.parse(
        deps.syncGraphFromState(deps.getState(), deps.getGraphMemory(), reason ? `vector_graph_sync:${reason}` : "vector_graph_sync", deps.now()),
      );
  ...
  deps.registerVectorTool(
    ...
    async ({ node_id, entity_type, search, limit }: any) => {
      const cleaned = deps.sanitizeRecursive({ node_id, entity_type, search, limit }) as any;

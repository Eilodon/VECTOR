...
### Turn 4 — Founder Fit (chỉ khi chưa rõ từ các câu trước)
...
> **Lưu ý:** Câu 4.2 là seed data cho Founder Edge Audit sẽ chạy chính thức ở Channel phase. Ghi nhận vào KB (founder_edge_audit section) ngay cả khi còn sơ bộ.
...
```
☐ Product mô tả được trong một câu
☐ Problem được đặt tên (không phải "inefficiency" hay "pain point" chung)
☐ Ít nhất 1 target persona được giả định
☐ Lý do "tại sao bây giờ" tồn tại (market, timing, hoặc personal trigger)
☐ Ít nhất 1 constraint được ghi nhận
...
Problem:              [một câu — pain, không phải feature]
...
Early network signals:      [có warm contacts ở target community? y/n + context]
Riskiest assumption going into ICP: [đặt tên nó]
```
...
- Set `milestone: M0 complete` và `gates.intake_cleared: true` trước khi move sang ICP.
...
Nếu founder chỉ trả lời một dòng và không muốn elaborrate:
...
Intake tốt không phải là hỏi nhiều câu — là hỏi đúng câu theo đúng thứ tự để founder không phòng thủ. Founder hay phòng thủ khi cảm thấy bị test. Hỏi về người thật và vấn đề thật trước khi hỏi về ý tưởng của họ — bởi vì họ tự tin về người thật hơn là về ý tưởng.
